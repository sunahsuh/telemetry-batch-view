...
case class HistogramRow(experiment_id: String, branch: String, subgroup: String, metric: Option[Map[Int, Int]]) {
  def toPreAggregateRow: PreAggHistogramRow = {
    import HistogramIntermediate._
...
class HistogramIntermediate(name: String, hd: HistogramDefinition, df: DataFrame)
  extends MetricIntermediate[Int](name, hd, df) {
