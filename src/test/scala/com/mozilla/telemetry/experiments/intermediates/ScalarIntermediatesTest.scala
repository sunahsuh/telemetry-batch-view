...
import com.mozilla.telemetry.experiments.analyzers.{HistogramPoint, MetricAnalysis}
...
class ScalarAnalyzerTest extends FlatSpec with Matchers with DatasetSuiteBase {
  ...
  "Uint Scalars" can "be aggregated" in {
    val df = fixture
    val aggregator = ScalarIntermediate.getIntermediateAggregator("uint_scalar",
      UintScalar(false, "name"),
      df.where(df.col("experiment_id") === "experiment1")
    )
    val actual = aggregator.aggregate.collect().toSet
  ...
  "Keyed Uint Scalars" can "be aggregated" in {
    val df = fixture
    val aggregator = ScalarIntermediate.getIntermediateAggregator("keyed_uint_scalar",
  ...
  "Boolean Scalars" can "be aggregated" in {
    val df = fixture
    val aggregator = ScalarIntermediate.getIntermediateAggregator("boolean_scalar",
  ...
  "Keyed Boolean Scalars" can "be aggregated" in {
    val df = fixture
    val aggregator = ScalarIntermediate.getIntermediateAggregator("keyed_boolean_scalar",
  ...
  "String Scalars" can "be aggregated" in {
    val df = fixture
    val aggregator = ScalarIntermediate.getIntermediateAggregator("string_scalar",
  ...
  "Keyed String Scalars" can "be aggregated" in {
    val df = fixture
    val aggregator = ScalarIntermediate.getIntermediateAggregator("keyed_string_scalar",
  ...
  "Invalid scalars" should "be filtered out" in {
    ...
    val actual = aggregator.aggregate.collect().filter(_.experiment_branch == "control").head
    assert(actual.n == 3L)

    val keyed_analyzer = ScalarIntermediate.getIntermediateAggregator("keyed_uint_scalar",
      UintScalar(true, "name"),
      df.where(df.col("experiment_id") === "experiment1")
    )
    val keyed_actual = aggregator.aggregate.collect().filter(_.experiment_branch == "control").head
    assert(keyed_actual.n == 3L)
  }

  "Uint Scalar Intermediates" can "be aggregated" in {
    val df = fixture
    val aggregator = ScalarIntermediate.getIntermediateAggregator("uint_scalar",
      UintScalar(false, "name"),
      df.where(df.col("experiment_id") === "experiment1")
  ...
  "Keyed Uint Scalar Intermediates" can "be aggregated" in {
    val df = fixture
    val aggregator = ScalarIntermediate.getIntermediateAggregator("keyed_uint_scalar",
      UintScalar(true, "name"),
      df.where(df.col("experiment_id") === "experiment1")
  ...
  "Boolean Scalar Intermediates" can "be aggregated" in {
    val df = fixture
    val aggregator = ScalarIntermediate.getIntermediateAggregator("boolean_scalar",
      BooleanScalar(false, "name"),
      df.where(df.col("experiment_id") === "experiment1")
  ...
  "Keyed Boolean Scalar Intermediates" can "be aggregated" in {
    val df = fixture
    val aggregator = ScalarIntermediate.getIntermediateAggregator("keyed_boolean_scalar",
      BooleanScalar(true, "name"),
      df.where(df.col("experiment_id") === "experiment1")
  ...
  "String Scalar Intermediates" can "be aggregated" in {
    val df = fixture
    val aggregator = ScalarIntermediate.getIntermediateAggregator("string_scalar",
      StringScalar(false, "name"),
      df.where(df.col("experiment_id") === "experiment1")
  ...
  "Keyed String Scalar Intermediates" can "be aggregated" in {
    val df = fixture
    val aggregator = ScalarIntermediate.getIntermediateAggregator("keyed_string_scalar",
      StringScalar(true, "name"),
      df.where(df.col("experiment_id") === "experiment1")
  ...
  "Invalid scalars" should "be filtered out in intermed" in {
    val df = fixture
    val aggregator = ScalarIntermediate.getIntermediateAggregator("uint_scalar",
      UintScalar(false, "name"),
      df.where(df.col("experiment_id") === "experiment1")
