...
class HistogramAnalyzerTest extends FlatSpec with Matchers with DatasetSuiteBase {
  ...
  "Non-keyed Histograms" can "be aggregated" in {
    val df = fixture
    val aggregator = new HistogramIntermediate("histogram",
  ...
  "Keyed Histograms" can "be aggregated" in {
    val df = fixture
    val aggregator = new HistogramIntermediate("keyed_histogram",
  ...
  "Unknown histogram" should "return an empty dataset" in {
    val df = fixture
    val aggregator = new HistogramIntermediate("unknown_histogram",
      EnumeratedHistogram(keyed = true, "name", 150),
      df.where(df.col("experiment_id") === "experiment1")
    )
    val actual = aggregator.aggregate
